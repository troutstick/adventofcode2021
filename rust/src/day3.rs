...
fn vec_to_bin(bin_vec: Vec<usize>) -> usize {
    let bin_value = |(index, val): (usize, &usize)| -> usize {
        *val * (1 << index)
    };
...
fn get_most_or_least_common(list_nums: &Vec<Vec<usize>>, index: usize, is_most_common: bool) -> usize {
    let get_ith = |v: &Vec<usize>| -> usize {
        v[index]
    };

    let filter_ones = |i: &usize| {
        *i == 1
    };
